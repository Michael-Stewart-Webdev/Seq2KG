...
class ModelEvaluator():
	...
	def __init__(self, model, dev_loader, word_vocab, wordpiece_vocab, hierarchy_tr, hierarchy_et, ground_truth_triples, cf):
		self.model = model 
		self.dev_loader = dev_loader 
		self.word_vocab = word_vocab 
		self.wordpiece_vocab = wordpiece_vocab
		self.hierarchy_tr = hierarchy_tr		
	...
	def evaluate_model(self, epoch, data_loader, mode="training"):
		...
		for (i, (batch_x, _, _, _, _, _, batch_tx, batch_ty_tr, batch_ty_et, batch_tm)) in enumerate(data_loader):
			...
			token_preds_tr, token_preds_et = self.model.predict_token_labels(joined_embs, token_idxs_to_wp_idxs)
		...
		tagged_sents = self.get_tagged_sents_from_preds(all_txs, all_tys, all_tys_et, all_preds, all_preds_et)
		...
		def build_true_and_preds(tys, preds):
			...
			for b, batch in enumerate(tys):
				for i, row in enumerate(batch):		
					...
					pred_cats = self.hierarchy_tr.onehot2categories(preds[b][i])
		...
		for idx, a in enumerate(predicted_triples):
			for t in a:
				triples_str += "%s %s\n" % (idx, ",".join(["[ " + " ".join(w) + " ]" if i >= 3 else " ".join(w) for i, w in enumerate(t)]))
		logger.info("\n" + triples_str)
	...
	def get_triples_from_tagged_sents(self, tagged_sents):		
		...
		def get_triples(tagged_sents):
			...
			for s in tagged_sents:
				label_mapping = {"head": 0, "rel": 1, "tail": 2, "type_head": 3, "type_tail": 4, "correct_type_head": 5, "correct_type_tail": 6}
				current_labels = set()
				triples     = [[ [] , [] , [], set(), set(), set(), set()] for x in range(10)]
				triple_idxs = [[ [] , [] , [] ] for x in range(10)]
				for word_idx, (word, correct_labels, predicted_labels, correct_et_labels, predicted_et_labels) in enumerate(s):
					...
					for label in ls:
						...
						if label in current_labels or len(triples[idx][label_mapping[label_type]]) == 0:
							...
							if len(triple_idxs[idx][label_mapping[label_type]]) == 0 or	triple_idxs[idx][label_mapping[label_type]][-1] == word_idx - 1:
									...
									if label_type != "rel":
										for p_et in predicted_et_labels:
											triples[idx][label_mapping["type_" + label_type]].add(p_et)
										for g_et in correct_et_labels:
											triples[idx][label_mapping["correct_type_" + label_type]].add(g_et)
	...
	def get_tagged_sents_from_preds(self, all_txs, all_tys, all_tys_et, all_preds, all_preds_et):
		tagged_sents = []
		for b, batch in enumerate(all_txs):
			...
			for i, token_ix in enumerate(batch):
				...
				tagged_sent.append([									\
					...
					[] if all_tys == [] else [l for l in self.hierarchy_tr.onehot2categories(all_tys[b][i]) if "/" in l],	\
					[l for l in self.hierarchy_tr.onehot2categories(all_preds[b][i]) if "/" in l],	\
					[] if all_tys_et == [] else [l for l in self.hierarchy_et.onehot2categories(all_tys_et[b][i]) if "/" in l],	\
					[l for l in self.hierarchy_et.onehot2categories(all_preds_et[b][i]) if "/" in l],	\
				])
	...
	def no_improvement_in_n_epochs(self, n, epoch):
		return epoch >= 15 and (epoch - self.best_score_and_epoch[1] >= n)
...
class TrainedEndToEndModel():
	...
	def __init__(self, dataset, cf):
		...
		self.modelEvaluator = ModelEvaluator(model, None, self.word_vocab, self.wordpiece_vocab, hierarchy_tr, hierarchy_et, None, cf)
		self.cf = cf
	...
	def triples_from_docs(self, docs):
		...
		for doc_idx, doc in enumerate(evaluation_docs):
			...
			for i, sent in enumerate(doc):
				...
				data_y_tr.append(np.asarray([]))
				data_y_et.append(np.asarray([]))
				data_z_tr.append(np.asarray([]))
				data_z_et.append(np.asarray([]))
				data_i.append(np.asarray(i))
				data_tx.append(np.asarray(sent.token_indexes)[:self.cf.MAX_SENT_LEN])
				data_ty_tr.append(np.asarray([]))
				data_ty_et.append(np.asarray([]))
		...
		for doc in triples:
			...
			for i, t in enumerate(doc):
				cleaned_triple = [clean(t[0]), clean(t[1]), clean(t[2]), t[3], t[4], t[5], t[6]]
				if not any([len(x) == 0 for x in cleaned_triple[:3]]):
