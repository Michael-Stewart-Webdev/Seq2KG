...
class ModelEvaluator():
	...
	def evaluate_model(self, epoch, data_loader, mode="training"):
		...
		for (i, (batch_idx, batch_doc_idx, batch_d, batch_h, batch_r, batch_t, batch_y)) in enumerate(data_loader):
			...
			#bbb = [str(t) for t in self.evaluation_dataset.iloc[i]]
			#if bbb[0] == "0":
			#	print(' '.join(bbb))
			#	print(batch_h[0][:3], batch_r[0][:3], batch_t[0][:3], batch_y)
			#	print("Prediction weights", preds, confidences)
			#	print()
			...
			#print([t for t in self.evaluation_dataset.iloc[batch_idx]])
