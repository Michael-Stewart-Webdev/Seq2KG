...
def train(model, data_loaders, word_vocab, wordpiece_vocab, hierarchy, ground_truth_triples, epoch_start = 1):
	...
	modelEvaluator = ModelEvaluator(model, data_loaders['dev'], word_vocab, wordpiece_vocab, hierarchy, ground_truth_triples, cf)
	...
	for epoch in range(epoch_start, max_epochs + 1):
		...
		for (i, (batch_x, batch_y, batch_z, _, batch_tx, _, _)) in enumerate(data_loaders["train"]):
...
def main(opts):
	...
	train(model, data_loaders, word_vocab, wordpiece_vocab, hierarchy, ground_truth_triples)
