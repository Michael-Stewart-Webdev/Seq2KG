	...
	for doc_idx in dataset_triples:
		...
		for k, m in enumerate(mentions_triples):
			...
			for l in m['labels']:
				...
				#Z	updated_labels.append(triple_part)
				l2 = triple_num + "/" + triple_part + "_" + num
		...
		if verbose:
			...
			for j, m in enumerate(mentions_et):
				start = m['start']
				end = m['end']
		...
		merged_data.append({'tokens': tokens, 'mentions': {'triples': mentions_triples, 'entity_types': mentions_et } })
